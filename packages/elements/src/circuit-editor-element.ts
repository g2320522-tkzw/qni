...
type CircuitEditorEvent =
  ...
  | {
      type: 'SHOW_OPERATION_MENU'
      operation: Operation
    }
  | {
      type: 'SHOW_OPERATION_INSPECTOR_IF'
      operation: Ifable
    }
  | {
      type: 'SHOW_OPERATION_INSPECTOR_ANGLE'
      operation: Angleable
    }
  | {
      type: 'SHOW_OPERATION_INSPECTOR_FLAG'
      operation: Flaggable
    }
  | {
      type: 'SET_OPERATION_IF'
      operation: Ifable
      if: string
    }
  | {
  ...
  | {
      type: 'RESIZE_HANDLE_IN_SNAP_RANGE'
      operation: Operation
      x: number
      y: number
    }
  | {
      type: 'MOUSE_ENTER_STEP'
      step: CircuitStepElement
    }
  | {
      type: 'MOUSE_LEAVE_STEP'
      step: CircuitStepElement
    }
  | {
...
export class CircuitEditorElement extends HTMLElement {
...

    {
      ...
      actions: {
        ...
        setOperationActive: (_context, event) => {
          if (event.type !== 'GRAB_OPERATION' && event.type !== 'GRAB_RESIZE_HANDLE') {
            return
          }
        ...
        snapOperationIntoDropzone: (_context, event) => {
          if (event.type !== 'OPERATION_IN_SNAP_RANGE') {
            return
          }
        ...
        snapResizeHandleIntoDropzone: (_context, event) => {
          if (event.type !== 'RESIZE_HANDLE_IN_SNAP_RANGE') {
            return
          }
        ...
        resizeCircuit: () => {
          ...
          this.dispatchEvent(
            new Event('circuit-editor-resize', {
              bubbles: true,
            }),
          )
        },
        ...
        setResizeHandleSnapTargets: (_context, event) => {
          if (event.type !== 'GRAB_RESIZE_HANDLE') {
            return
          }
        ...
        activateStep: (_context, event) => {
          if (event.type !== 'MOUSE_ENTER_STEP' && event.type !== 'SNAP_STEP') {
            return
          }
        ...
        deactivateStep: (_context, event) => {
          if (event.type !== 'MOUSE_LEAVE_STEP' && event.type !== 'UNSNAP_STEP') {
            return
          }
        ...
        maybeDisableAllInspectorPanes: (_context, event) => {
          if (event.type !== 'END_DRAGGING_OPERATION') {
            return
          }
          ...
          if (this.inspectorButton === undefined) {
            return
          }
        ...
        initOperationMenu: (_context, event) => {
          if (event.type !== 'DROP_OPERATION') {
            return
          }

          const operation = event.operation

          if (isMenuable(operation)) {
            operation.initMenu()
          }
        },

        showOperationMenu: (_context, event) => {
          if (event.type !== 'SHOW_OPERATION_MENU') {
            return
          }
        ...
        showOperationInspectorIf: (_context, event) => {
          if (event.type !== 'SHOW_OPERATION_INSPECTOR_IF') {
            return
          }
        ...
        showOperationInspectorAngle: (_context, event) => {
          if (event.type !== 'SHOW_OPERATION_INSPECTOR_ANGLE') {
            return
          }
        ...
        showOperationInspectorFlag: (_context, event) => {
          if (event.type !== 'SHOW_OPERATION_INSPECTOR_FLAG') {
            return
          }
        ...
        setOperationIf: (_context, event) => {
          if (event.type !== 'SET_OPERATION_IF') {
            return
          }
        ...
        setOperationAngle: (_context, event) => {
          if (event.type !== 'SET_OPERATION_ANGLE') {
            return
          }
        ...
        setOperationFlag: (_context, event) => {
          if (event.type !== 'SET_OPERATION_FLAG') {
            return
          }
        ...
        maybeUpdateOperationInspector: (_context, event) => {
          if (event.type !== 'ACTIVATE_OPERATION') {
            return
          }
  ...
  private activateOperation(event: Event): void {
    ...
    if (!isOperation(operation)) {
      throw new Error(`${operation} must be an Operation.`)
    }
  ...
  private showOperationInspectorIf(event: Event): void {
    ...
    if (!isIfable(operation)) {
      throw new Error(`${operation} isn't an Ifable Operation.`)
    }
  ...
  private showOperationInspectorAngle(event: Event): void {
    ...
    if (!isAngleable(operation)) {
      throw new Error(`${operation} isn't an Angleable Operation.`)
    }
  ...
  private showOperationInspectorFlag(event: Event): void {
    ...
    if (!isFlaggable(operation)) {
      throw new Error(`${operation} isn't a Flaggable Operation.`)
    }
  ...
  private setOperationAngle(event: Event): void {
    ...
    if (!isAngleable(operation)) {
      throw new Error(`${operation.outerHTML}: [data-angle] not found.`)
    }
  ...
  private clickStep(event: Event): void {
    ...
    if (!isCircuitStepElement(step)) {
      throw new Error(`${step} isn't a circuit-step.`)
    }
